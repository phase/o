...
S rdln(){L z=0,n=0;S r=alc(BZ);r[0]=0;while(fgets(r+z,BZ,stdin)){z+=strlen(r+z);if((n=r[z-1]=='\n'))BK;r=rlc(r,z+BZ);}if(!n&&!feof(stdin))PXE;if(n)r[z-1]=0;if(z>1&&r[z-2]=='\r')r[z-2]=0;R r;} //read line
...
V key(ST st){
    I i;O b=pop(st);if(b->t==TA){O t=pop(b->a);psh(st,b);psh(st,t);R;}
    O a=top(st);if(b->t==TD&&(a->t==TA||a->t==TS)){I i=b->d,s=a->t==TS;if(i<0||i>=(s?a->s.z:len(a->a)))ex("out of range");psh(st,s?newosc(a->s.s[i]):dup(a->a->st[i]));dlo(b);}
    else TE;} //key
